...
bool useThemes = false;
bool UGXPThemes::drawEdgeBorder = true;
UGXPThemeStyles UGXPThemes::m_style = Style1;
bool UGXPThemes::UseThemes()
{
	return useThemes;
}
void UGXPThemes::UseThemes(bool use)
{
	useThemes = use;
}
...
bool UGXPThemes::GetThemeRect(LPCWSTR theme, int iPartId, int iStateId, RECT *pRect)
{
	...
	if (useThemes)
	{
		...
		if (themeHandle)
		{
			const int TMT_DEFAULTPANESIZETemp = 5002;
			HRESULT hr = GetThemeRect(themeHandle, iPartId, iStateId, TMT_DEFAULTPANESIZETemp, pRect);
			success = SUCCEEDED(hr);
		}
	}
	return success;
