...
BEGIN_MESSAGE_MAP(CPropTreeItemCombo, CComboBox)
	//{{AFX_MSG_MAP(CWAFPubMFCdllItemCombo)
...
/////////////////////////////////////////////////////////////////////////////
// CWAFPubMFCdllItemCombo message handlers
...
void CPropTreeItemCombo::DrawAttribute(CDC* pDC, const RECT& rc)
{
	...
	if (!IsWindow(m_hWnd))
	{
		TRACE0("CWAFPubMFCdllItemCombo::DrawAttribute() - The window has not been created\n");
		return;
	}
...
BOOL CPropTreeItemCombo::CreateComboBox(DWORD dwStyle)
{
	...
	{
		TRACE0("CWAFPubMFCdllItemCombo::CreateComboBox() - failed to create combo box\n");
		return FALSE;
	}
...
int CPropTreeItemCombo::AddStringEx(LPCTSTR lpszString, int nTypeId)
{
	int nPos = AddString(lpszString);
	...
	SetItemData(nPos, (DWORD_PTR)nTypeId);
...
BOOL CPropTreeItemCombo::CreateComboBoxBool()
{
	...
	if (!Create(dwStyle, CRect(0,0,0,0), m_pProp->GetCtrlParent(), GetCtrlID()))
	{
		TRACE0("CWAFPubMFCdllItemCombo::CreateComboBoxBool() - failed to create combo box\n");
		return FALSE;
	}
