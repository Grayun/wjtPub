// WAFPubMFCdllList.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CWAFPubMFCdllList message handlers
...
void CPropTreeList::OnPaint() 
{
	...
	// draw expand column
	memdc.SelectObject(GetSysColorBrush(COLOR_BTNFACE));
	memdc.PatBlt(0, 0, WAFPUBMFCDLLITEM_EXPANDCOLUMN, rc.Height(), PATCOPY);
...
void CPropTreeList::OnLButtonDown(UINT, CPoint point) 
{
	...
	switch (nHit)
	{
		case HTCOLUMN:
			...
			pDC = GetDC();
			GetClientRect(rc);
			pDC->PatBlt(m_nPrevCol - WAFPUBMFCDLLITEM_COLRNG/2, 0, WAFPUBMFCDLLITEM_COLRNG, rc.bottom, PATINVERT);
			ReleaseDC(pDC);
			break;
		...
		default:
			if ((pItem = m_pProp->FindItem(point))!=NULL)
			{
				CPropTreelItem* pOldFocus = m_pProp->GetFocusedItem();
...
void CPropTreeList::OnMouseMove(UINT, CPoint point)
{
	if (m_bColDrag)
	{
		...
		GetClientRect(rc);
		pDC->PatBlt(m_nPrevCol - WAFPUBMFCDLLITEM_COLRNG/2, 0, WAFPUBMFCDLLITEM_COLRNG, rc.bottom, PATINVERT);
		pDC->PatBlt(point.x - WAFPUBMFCDLLITEM_COLRNG/2, 0, WAFPUBMFCDLLITEM_COLRNG, rc.bottom, PATINVERT);
		m_nPrevCol = point.x;
		ReleaseDC(pDC);
	}
}
