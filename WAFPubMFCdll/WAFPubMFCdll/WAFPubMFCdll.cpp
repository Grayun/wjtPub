// WAFPubMFCdll.cpp : implementation file
//
...
#define WAFPUBMFCDLLITEM_COLRNG				5			// width of splitter

static AFX_EXTENSION_MODULE WAFPubMFCdllDLL = {NULL, NULL};
...
extern "C" int APIENTRY DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID)
{
	...
	{
		TRACE0("WAFPUBMFCDLL.DLL Initializing!\n");
		
		if (!AfxInitExtensionModule(WAFPubMFCdllDLL, hInstance))
			return 0;

		new CDynLinkLibrary(WAFPubMFCdllDLL);

		ghInst = hInstance;
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("WAFPUBMFCDLL.DLL Terminating!\n");
		AfxTermExtensionModule(WAFPubMFCdllDLL);
	}
...
CPropTreelItem* CPropTree::s_pFound;
...
CPropTree::~CPropTree()
{
	...
	// free global resource when ALL CWAFPubMFCdlls are destroyed
	if (!s_nInstanceCount)
		FreeGlobalResources();
}
...
BOOL CPropTree::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
	...
	AfxSetResourceHandle(ghInst);
	HCURSOR hCursor = ::LoadCursor(NULL, IDC_ARROW);
	LPCTSTR pszCreateClass = AfxRegisterWndClass(CS_HREDRAW|CS_VREDRAW, hCursor);

	bool bRet = pWnd->Create(pszCreateClass, _T(""), dwStyle, rect, pParentWnd, nID);
...
int CPropTree::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		TRACE0("Failed to create CWAFPubMFCdllList\n");
		return -1;
	}
...
CPropTreelItem* CPropTree::GetFocusedItem()
{
	return m_pFocus;
}


CPropTreelItem* CPropTree::GetRootItem()
{
	return &m_Root;
}
...
CPropTreelItem* CPropTree::GetVisibleList()
{
	return m_pVisbleList;
}


void CPropTree::AddToVisibleList(CPropTreelItem* pItem)
{
	if (!pItem)
		return;
...
BOOL CPropTree::EnumItems(CPropTreelItem* pItem, ENUMPROPITEMPROC proc, LPARAM lParam)
{
	if (!pItem || !proc)
		return FALSE;
...
void CPropTree::DeleteItem(CPropTreelItem* pItem)
{
	Delete(pItem);
	UpdatedItems();
}
...
void CPropTree::SetColumn(LONG nColumn)
{
	...
	if (rc.IsRectEmpty())
		nColumn = __max(WAFPUBMFCDLLITEM_EXPANDCOLUMN, nColumn);
	else
		nColumn = __min(__max(WAFPUBMFCDLLITEM_EXPANDCOLUMN, nColumn), rc.Width() - WAFPUBMFCDLLITEM_EXPANDCOLUMN);
...
void CPropTree::Delete(CPropTreelItem* pItem)
{
...
void CPropTree::SetFocusedItem(CPropTreelItem* pItem)
{
	m_pFocus = pItem;
...
BOOL CPropTree::IsItemVisible(CPropTreelItem* pItem)
{
	if (!pItem)
		return FALSE;

	for (CPropTreelItem* pNext = m_pVisbleList; pNext; pNext = pNext->GetNextVisible())
	{
		if (pNext==pItem)
			return TRUE;
	}
...
void CPropTree::EnsureVisible(CPropTreelItem* pItem)
{
	if (!pItem)
		return;
...
CPropTreelItem* CPropTree::InsertItem(CPropTreelItem* pItem, CPropTreelItem* pParent)
{
	if (!pItem)
		return NULL;
...
LONG CPropTree::HitTest(const POINT& pt)
{
	...
	{
		if (!pItem->IsRootLevel() && pt.x >= m_Origin.x - WAFPUBMFCDLLITEM_COLRNG && pt.x <= m_Origin.x + WAFPUBMFCDLLITEM_COLRNG)
			return HTCOLUMN;

		if (pt.x > m_Origin.x + WAFPUBMFCDLLITEM_COLRNG)
			return HTATTRIBUTE;
...
CPropTreelItem* CPropTree::FindItem(const POINT& pt)
{
	CPropTreelItem* pItem;
...
CPropTreelItem* CPropTree::FindItem(UINT nCtrlID)
{
	s_pFound = NULL;
...
BOOL CALLBACK CPropTree::EnumFindItem(CPropTree*, CPropTreelItem* pItem, LPARAM lParam)
{
	ASSERT(pItem!=NULL);
...
void CPropTree::SelectItems(CPropTreelItem* pItem, BOOL bSelect)
{
	if (!pItem)
		pItem = &m_Root;
...
CPropTreelItem* CPropTree::FocusFirst()
{
	CPropTreelItem *pold;
...
CPropTreelItem* CPropTree::FocusLast()
{
	CPropTreelItem* pNext;
	CPropTreelItem* pChange;
...
CPropTreelItem* CPropTree::FocusPrev()
{
	CPropTreelItem* pNext;
	CPropTreelItem* pChange;
...
CPropTreelItem* CPropTree::FocusNext()
{
	CPropTreelItem* pNext;
	CPropTreelItem* pChange;
...
void CPropTree::RefreshItems(CPropTreelItem* pItem)
{
	if (!pItem)
		pItem = &m_Root;
...
BOOL CALLBACK CPropTree::EnumSelectAll(CPropTree*, CPropTreelItem* pItem, LPARAM lParam)
{
	if (!pItem)
		return FALSE;
...
BOOL CALLBACK CPropTree::EnumRefreshAll(CPropTree*, CPropTreelItem* pItem, LPARAM)
{
	if (!pItem)
		return FALSE;
...
BOOL CALLBACK CPropTree::EnumMoveAll(CPropTree*, CPropTreelItem* pItem, LPARAM)
{
	if (!pItem)
		return FALSE;
...
LRESULT CPropTree::SendNotify(UINT nNotifyCode, CPropTreelItem* pItem)
{
	if (!IsWindow(m_hWnd))
		return 0L;
