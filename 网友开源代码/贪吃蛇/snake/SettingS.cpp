...
void CSettingS::SetInfo(HWND hWnd, Image *pImage, int nWidth, int nHeight)
{
	m_hWnd = hWnd;
	m_pImage = pImage;
...
void CSettingS::Draw(CDC &dc, CRect rcUpdate)
{
	if(!m_bDrawBackDC)
	{
		m_bDrawBackDC = true;
		...
		//标题
		graphics.DrawImage(m_pImage, Rect(m_rcTitle.left, m_rcTitle.top, m_rcTitle.Width(), m_rcTitle.Height()), 820, 410, 400, 170, UnitPixel);

		graphics.DrawImage(m_pImage, Rect(m_rcTitle.left + m_nWidth, m_rcTitle.top, m_rcTitle.Width(), m_rcTitle.Height()), 820, 410, 400, 170, UnitPixel);
		...
		graphics.DrawImage(m_pImage, Rect(m_rcTitle.left + m_nWidth * 2, m_rcTitle.top, m_rcTitle.Width(), m_rcTitle.Height()), 820, 410, 400, 170, UnitPixel);
...
bool CSettingS::OnMouseMove(UINT nFlags, CPoint point)
{
...
bool CSettingS::OnLButtonDown(UINT nFlags, CPoint point)
{
	int nOldItemHover = m_nItemHover;
	if(m_rcTitle.PtInRect(point))
	{
...
bool CSettingS::OnLButtonUp(UINT nFlags, CPoint point)
{
	int nOldItemHover = m_nItemHover;
	if(m_rcTitle.PtInRect(point))
	{
	...
	{
		::SendMessage(m_hWnd, MSG_SETTING, m_nItemDown, 0);
	}
...
bool CSettingS::OnTimer(UINT_PTR nIDEvent)
{
	return false;
}
